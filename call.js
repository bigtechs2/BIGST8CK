    ...
    bot.ev.on("Events", async (call) => {
        ...
        if (!config.system.restrict) {
            ...
            if (reportOwners && reportOwners.length > 0) {
                ...
                for (let i = 0; i < reportOwners.length; i++) {
                    await bot.sendMessage(reportOwners[i] + Baileys.S_WHATSAPP_NET, {
                        text: bot.format.info(`Account @${fromPnId} has been automatically banned for reasons ${bot.msg.inlineCode("Anti Call")}.`),
                        mentions: [fromPnJid]
                    });
            ...
            await bot.sendMessage(fromJid, {
                text: bot.format.info("You have been automatically banned for breaking the rules!"),
                buttons: [{
                    text: "Contact Owner",
