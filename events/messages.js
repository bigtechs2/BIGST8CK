...
async function WarningHandler(ctx, senderJid, senderId, senderLid, groupJid, groupDb) {
    ...
    await ctx.reply({
        text: ctx.format.info(`Warning ${currentWarnings}/${maxWarnings} for @${ctx.getId(senderId)}!`),
        mentions: [senderJid]
    });
    ...
    if (currentWarnings >= maxWarnings) {
        ...
        if (isBotAdmin) {
            await ctx.reply(ctx.format.info(`You have received ${maxWarnings} warnings and will be removed from the group!`));
            if (!config.system.restrict) await ctx.group().kick(senderJid);
            groupDb.warnings = warnings.filter(warning => warning.jid !== senderLid);
        } else {
            await ctx.reply(ctx.format.info(`${config.msg.botAdmin} Cannot remove you who have reached ${maxWarnings} warnings.`));
        }
    }
    groupDb.save();
}
        ...
        if (isGroup || isPrivate) {
            ...
            if (senderAfk?.reason || senderAfk?.timestamp) {
                ...
                if (timeElapsed > 3000) {
                    ...
                    await ctx.reply(ctx.format.info(`You have exited AFK ${senderAfk.reason ? `with reason ${ctx.format.inlineCode(senderAfk.reason)}` : "without reason"} for ${timeago}.`));
                    senderDb.afk = {};
                    senderDb.save();
                }
            }
                ...
                if (afkMentions.length > 0) {
                    for (const afkMention of afkMentions) {
                        ...
                        if (mentionAfk.reason || mentionAfk.timestamp) {
                            ...
                            await ctx.reply({
                                text: ctx.format.info(`Do not disturb! @${ctx.getId(afkMention)} is AFK ${mentionAfk.reason ? `with reason ${ctx.format.inlineCode(mentionAfk.reason)}` : "without reason"} for ${timeago}.`),
                                mentions: [afkMention]
                            });
                        }
                    }
                ...
                if (!isCmd && !isOwner && !isAdmin) {
                    ...
                    for (const type of antiTypes) {
                        if (groupDb?.option?.[`anti${type}`]) {
                            ...
                            if (!!isMedia) {
                                await ctx.reply(ctx.format.info(`Don't send ${type}!`));
                    ...
                    if (groupDb?.option?.antigcsw) {
                        ...
                        if (isMedia) {
                            await ctx.reply(ctx.format.info("Don't create SW in the group, no one cares!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antilink) {
                        if (msg.body && ctx.helper.isUrl(msg.body)) {
                            await ctx.reply(ctx.format.info("Don't send links!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antispam) {
                        ...
                        if (newCount > 5) {
                            await ctx.reply(ctx.format.info("Don't spam, it's lagging, dude!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antitagsw) {
                        ...
                        if (isMedia) {
                            await ctx.reply(ctx.format.info("Don't tag the group in SW, no one cares!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antitoxic) {
                        ...
                        if (msg.body && toxicRegex.test(msg.body)) {
                            await ctx.reply(ctx.format.info("Don't be toxic!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
