...
class Ctx {
    ...
    get sender() {
        return {
            ...this._sender,
            isOwner: () => helper.checkOwner(this._sender.phoneNumber || this._sender.jid, this._self.owner, this._msg.key.fromMe)
        };
    }
    ...
    get db() {
        ...
        return {
            ...
            user: helper.getDb(users, this._sender.jid),
    ...
    async target(priority = ["quoted", "mentioned", "text"]) {
        const strategies = {
            ...
            quoted: () =>
                this.quoted?.sender?.jid && {
                    id: this.quoted.sender.jid,
                    source: "quoted"
                }
        };
    ...
    async bio(text, options) {
        await this._client.updateTextStatus(text, options);
    }
    async fetchBio(jid = this._sender.jid) {
        const bio = await this._client.fetchTextStatus(jid);
        return bio?.[0] || null;
    }
    ...
    async edit(newText, key) {
        return await this.editMessage(this.id, newText, key);
    }
    ...
    checkOwner(jid = this._sender.jid, fromMe = false) {
